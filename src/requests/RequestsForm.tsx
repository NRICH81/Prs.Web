...
import type { IRequest } from "./IRequests";
import type { IUser} from "../users/IUsers";
...
function RequestsForm() {
  ...
  const save: SubmitHandler<IRequest> = async (requests) => {
    try {
      delete requests.user;
      if (!requests.id) {
        const newRequests = await requestAPI.post(requests);
        navigate(`/requests/detail/${newRequests.id}`);
      } else {
        await requestAPI.put(requests);
